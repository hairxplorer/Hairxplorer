...
# class AnalysisRequest(BaseModel): #Inutile pour /update-config , supprimé
#     api_key: str
...
def get_db_connection():
    ...
    # Forcer la suppression du fichier (SEULEMENT POUR LE TEST)
    if os.path.exists(DATABASE_PATH):
        try:
            os.remove(DATABASE_PATH)
            print("DEBUG: config.db supprimé avec succès.")
        except Exception as e:
            print(f"DEBUG: Erreur lors de la suppression de config.db: {e}")
